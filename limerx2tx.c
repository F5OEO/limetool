...
void print_usage()
{
	...
	fprintf(stderr, \
		"limerx2tx -%s\n\
Usage:\nlimerx2tx -s SymbolRate  [-f Frequency input(Khz)] [-o Frequency output(Khz)] [-g Gain] [-t SampleType] [-h] \n\
-s            SymbolRate in KS \n\
-f            Frequency in (Khz)\n\
-t            Frequency out (Khz)\n\
-g            Gain 0 to 100\n\
...
int main(int argc, char **argv) 
{
	...
	double RxFrequency=0,TxFrequency=0;
	int Gain = 50;
	int a;
	int anyargs = 0;
	...
	while (1)
	{
		a = getopt(argc, argv, "i:s:f:t:g:h");
		...
		case 's': // SymbolRate in KS
			...
			break;
		case 'f': // RxFrequency in Khz
			RxFrequency = atof(optarg)*1000.0;
			break;
       	case 't': // TxFrequency in Khz
			TxFrequency = atof(optarg)*1000.0;
			break;
		...
		case '?':
			if (isprint(optopt))
			{
				fprintf(stderr, "limerx2tx `-%c'.\n", optopt);
			}
			else
			{
				fprintf(stderr, "limerx2tx: unknown option character `\\x%x'.\n", optopt);
			}
			print_usage();
	...
	if ((TxFrequency == 0)||(RxFrequency == 0)) {
		fprintf(stderr, "Need set frequency in and out\n"); exit(0);
	}
	if (SymbolRate == 0) {
	...
	limesdr_set_rxfreq(RxFrequency);
	limesdr_receive();
	
    fprintf(stderr,"Rx %f Mhz-> Tx %f Mhz\n",RxFrequency/1e6,TxFrequency/1e6);
